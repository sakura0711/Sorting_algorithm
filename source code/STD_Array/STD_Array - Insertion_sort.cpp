...
void Print_Time(double TimeAdd, int i, string TypeKeys) {
    fstream outfile;
    outfile.open("C:\\Users\\MSI GP65\\source\\repos\\Data_structure - 排序法\\Sort - CostTime\\Time - STD_Array-Insertion_sort.txt", ios::app);
    if (TimeAdd < 1000)
    {
        outfile << "\n"
            << "(" << TypeKeys << ")"
...
void Insertion_sort(array<T, nSize>* pData, string TypeKeys) noexcept
{
    ...
    for (int i = 1; i < nSize; i++)
    {
        StartTime();
        temp = index[i];
        for (k = i; k > 0 && temp < index[k]; k--)
        {
            index[k] = index[k - 1]; // 比前面小就跟前面的交換
        }
        index[k] = temp;
        TimeAdd += GetTime();
...
#pragma region 10萬筆資料(各類型)
    ...
    Insertion_sort(IntData_1HT, "Int");
    delete IntData_1HT;

    //Sleep(5000);  // 暫停5秒(方便區別不同的資料型態)

    array<long int, 100000>* LongintData_1HT = new array<long int, 100000>;
    Produce_random(LongintData_1HT, 'I');
    Insertion_sort(LongintData_1HT, "Long int");
    ...
    Insertion_sort(FloatData_1HT, "Float");
    ...
    Insertion_sort(DoubleData_1HT, "Double");
    ...
    Insertion_sort(StringData_1HT, "String");
...
#pragma region 40萬筆資料(各類型)
    ...
    Insertion_sort(IntData_4HT, "Int");
    delete IntData_4HT;
    ...
    Insertion_sort(LongintData_4HT, "Long int");
    ...
    Insertion_sort(FloatData_4HT, "Float");
    ...
    Insertion_sort(DoubleData_4HT, "Double");
    ...
    Insertion_sort(StringData_4HT, "String");
